...
## 교착상태(deadlock)
- 자원을 가지고 있으면서 상대방이 가진 자원을 요구하고 있는 상황, 상대방도 마찬가지
- Deadlock
...
- request dege의 assignment edge 변경시 (점선을 포함하여) cycle이 생기지 않는 경우에만 요청 자원을 할당한다
- Cycle 생성 여부 조사시 프로세스가 수가 n일 때 O(n2) 시간이 걸린다
...
## 질문
1. 교착상태(Deadlock)의 발생 조건 4가지를 나열하고, 이 중 가장 비효율적인 해결 방법이 무엇인지 그 이유와 함께 설명하세요.
2. Deadlock의 예방(Prevention)과 회피(Avoidance)의 근본적인 차이점은 무엇인가요? 또한 실제 운영체제에서는 왜 둘 다 잘 사용하지 않는지(Ignorance) 설명하세요.
